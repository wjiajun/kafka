...
class ControllerChannelManager(controllerContext: ControllerContext,
  ...
  protected val brokerStateInfo = new HashMap[Int, ControllerBrokerStateInfo] // key broker id
  ...
  /**
   * 停止所有的RequestSendThread线程，并清理必要的资源
   */
  def shutdown() = {
    brokerLock synchronized {
  ...
  /**
   * 添加目标Broker 到brokerStateInfo数据结构中，并创建必要的资源，最后启动该线程。
   *
   * @param broker
   */
  def addBroker(broker: Broker): Unit = {
    ...
    brokerLock synchronized {
      ...
      if (!brokerStateInfo.contains(broker.id)) {
        // 将新的broker接入controller管理，并创建对应的RequestSendThread线程
        addNewBroker(broker)
  ...
  /**
   * 从brokerStateInfo移除目标broker相关数据
   *
   * @param brokerId
   */
  ...
  // 要为目标 Broker 创建一系列的配套资源，比如，NetworkClient 用于网络 I/O 操作、messageQueue 用于阻塞队列、requestThread 用于发送请求
  ...
  protected def startRequestSendThread(brokerId: Int): Unit = {
    // 获取broker 专属的RequestSendThread线程
...
case class QueueItem(apiKey: ApiKeys, request: AbstractControlRequest.Builder[_ <: AbstractControlRequest],// 阻塞队列上 AbstractControlRequest 类型
                     callback: AbstractResponse => Unit, enqueueTimeMs: Long)

// controller 发送元数据到其他broker 线程，每个broker一个
class RequestSendThread(val controllerId: Int,// controller 所在的broker 的 id
                        val controllerContext: ControllerContext,// controller 元数据信息
                        val queue: BlockingQueue[QueueItem],// 请求阻塞队列
                        val networkClient: NetworkClient,// 执行网络发送的io类
  ...
  override def doWork(): Unit = {
    ...
    try {
      ...
      if (clientResponse != null) {
        ...
        // 此Response的请求类型必须是LeaderAndIsrRequest、StopReplicaRequest或Upda
